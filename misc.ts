...
export function isQueuable(x): x is Queueable {
    return typeof x === "object" && x.hasOwnProperty("queue") && x.hasOwnProperty("queueTotalSize")
}
...
export function EnqueueValueWithSize(container: Queueable, value: any, size: number) {
    Assert(isQueuable(container));
