import {
  ...
  WritableStream, WritableStreamAbort, WritableStreamAddWriteRequest,
  WritableStreamCloseQueuedOrInFlight
} from "./writable_stream.ts";
import {defer, Defer, PromiseState, rejectDefer, rejectDefer, rejectDefer, resolveDefer} from "./defer";
import {Assert} from "./util";
import {
  WritableStreamDefaultControllerClose, WritableStreamDefaultControllerGetChunkSize,
  WritableStreamDefaultControllerGetDesiredSize, WritableStreamDefaultControllerWrite
...
export class WritableStreamDefaultWriter implements WritableStreamWriter {
  ...
  get closed(): Promise<undefined> {
    if (!IsWritableStreamDefaultWriter(this)) {
      return Promise.reject(new TypeError("this is not WritableStreamDefaultWriter"))
    }
...
export function IsWritableStreamDefaultWriter(x): x is WritableStreamDefaultWriter {
  return typeof x === "object" && x.hasOwnProperty("ownerWritableStream")
}

export function WritableStreamDefaultWriterAbort(writer: WritableStreamDefaultWriter, reason) {
  Assert(writer.ownerWritableStream !== void 0)
  return WritableStreamAbort(writer.ownerWritableStream, reason)
}

export function WritableStreamDefaultWriterClose(writer: WritableStreamDefaultWriter): Promise<any> {
...
export async function WritableStreamDefaultWriterCloseWithErrorPropagation(writer: WritableStreamDefaultWriter) {
  const stream = writer.ownerWritableStream
  Assert(stream !== void 0)
  const {state} = stream
...
export function WritableStreamDefaultWriterEnsureClosedPromiseRejected(writer: WritableStreamDefaultWriter, error) {
...
export function WritableStreamDefaultWriterEnsureReadyPromiseRejected(writer: WritableStreamDefaultWriter, error) {
...
export async function WritableStreamDefaultWriterWrite(writer: WritableStreamDefaultWriter, chunk) {
  const stream = writer.ownerWritableStream
  Assert(stream !== void 0)
  ...
  if (WritableStreamCloseQueuedOrInFlight(stream) || state === "closed") {
    throw new TypeError(`stream is ${state === "closed" ? "closed" : "closing"}`)
  }
  if (state === "erroring") {
    throw stream.storedError
  }
